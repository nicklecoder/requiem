...
**Each statement's window is fixed before adjudication is considered.** This is what makes the queue finite, and it was not always so. The sweep originally ranked each statement's *unadjudicated* partners and took the nearest k, which meant judging a pair promoted partner k+1 into the window and the outstanding count never fell: fifteen verdicts against this project's own corpus moved it from 93 to 92, and the field report saw 415 to 425 after 97 verdicts. Choosing the nearest k first and dropping the judged ones afterwards means a statement stops contributing once its window is clear, so verdicts strictly drain the queue.

The cost is deliberate: a pair at depth k+1 is no longer offered merely because a nearer pair was judged. Depth becomes a dial — sweep at `--neighbors 2`, then again at 5 — which is a decision the reader makes rather than a dribble the tool decides. Ordering is also the only model-independent lever available here: a similarity floor would have to name an absolute cosine, and what counts as close is a property of the embedding model rather than of the corpus.

Progress is therefore reported as two numbers, not one: pairs outstanding, and how many statements have been swept at the current depth. The second is the one that rises as work is done, which is what a reader needs in order to tell progress from motion.

**Candidates come from shared identifiers first, then from similarity.** Two statements naming `provider_venue_id` are talking about the same thing whatever words they wrapped it in, and that is precisely what embeddings cannot find: on a real 255-statement corpus, nearest-neighbour search over 1,936 candidate pairs missed three genuine conflicts, every one of which shared an identifier while sharing almost no prose. Pairs sharing a source file rank with them — two decisions about one piece of code. Identifier pairing needs no vectors, so a corpus is refused as unsweepable only when it has neither vectors nor identifiers.

**Opposed modality is a tiebreak, not a ranking.** Ranking by it pushed 48 unrelated pairs into the top 50 of a real audit, because a `must` set against a `must_not` on different subjects is ordinary and the opposition alone says nothing. It stays a flag, which is all the signal is worth.
...
Several fields are *derived at read time and never written to the file*: `stale` (code-derived provenance rehashed against current source), `embedding_status` (`missing` / `stale` / `fresh`, from comparing the stored vector's `source_hash` against the current body), and everything `check` computes about a result — `rank`, `verdict`, `similarity`, `shared_facets`, and `challenged`. None is content, so none passes through the stage/commit flow: a fact about a statement is not an edit to it.

`stale` travels with every `check` result, not only `get`. The hash was already stored and already compared on one path, which left the anti-rot mechanism reaching only a reader who had already chosen a candidate. `challenged` marks a statement a *proposed* statement contradicts or would supersede — a corpus of settled decisions makes existing decisions easy to honour, which is the point and also the risk.
...
**Relationship**: `from`, `to`, `type` (`conflicts_with`, `supersedes`, `depends_on`, `refines`, `duplicates`, `moved_to`), `note`. `duplicates` records an agent's verdict on a candidate pair so it stops resurfacing, and `moved_to` marks the stub `mv --leave-link` leaves behind.

`not_related` used to be a seventh type and is no longer written, though it is still read so an older corpus parses. A dismissal asserts only that somebody looked at a pair, and one real corpus accumulated 75 such edges — permanent noise in a structure people read to understand how decisions relate. Dismissals now live in a **verdict store**: one file per pair under `.requiem/verdicts/`, indexed like everything else, excluded from the sweep exactly as a relationship is, and revocable with `dismiss --restore`. Real findings stay edges, because they say something about the decisions themselves. (`scoped_to` was considered and dropped — namespace already expresses what a statement applies to; a redundant statement-to-statement edge for the same thing wasn't worth the extra relationship type.)
...
- Lives in its own file, `.requiem/statements/<namespace>/<id>.rejected.md`, beside that namespace's statement files. It was originally an append-only `_rejected.md` per namespace, on the reasoning that a lighter-weight record needs no file of its own. That was wrong in a way only use revealed: a shared file left an individual rejection unaddressable — `discard` resolved only `<id>.md` and so could never remove one, `mv` never rewrote its `see_instead`, `update` had no way to correct one entry — and two agents rejecting different ideas in one namespace wrote to the same file. Legacy `_rejected.md` files are still read, so an older corpus keeps working; a record edited out of one is migrated to its own file as part of the write.
- Embedded like a statement, keyed by `(source_kind, full_id)` since the two may share an id. Without a vector a rejection could only score the lexical half of the fusion, so every statement outranked every rejection under `--semantic`: measured on a 255-statement corpus, a query describing an already-rejected idea returned no rejections in the top ten and left the exact match at position 37 of 55 — in the one command whose documented purpose is to surface rejections first.
- `see_instead` is validated on write, rewritten when the statement it names moves, and reported by `audit` when it names nothing. It used to break in silence.
...
| `add` | `--id --namespace --kind --body [--modality] [--status] [--abstract] [--tags] [--provenance] [--source file:line] [--duplicate-ok]` | created statement. Runs `check` against the body first and refuses when an existing record reads as a duplicate, naming it; `--duplicate-ok` records it anyway. Lexical and identifier evidence only, never a network call: a write path that hangs when an endpoint is down is worse than one that misses a differently-worded duplicate |
| `update` | `<id> --body [--status] [--modality] [--abstract\|--no-abstract]`, or `<id> --rejection [--body] [--see-instead\|--clear-see-instead]` | updated statement or rejection. A rejection still in a legacy `_rejected.md` is migrated to its own file as part of the edit |
...
| `unlink` | `<from-id> <to-id> [--type]` | the statement with the edge removed, and the removed types on stderr. No `--type` removes every relationship joining the pair. Relationships are directional, so an unlink in the wrong direction errors naming the reverse edge rather than deleting one the caller did not name. Retyping is `unlink` then `link`: a pair may carry two types, so a replacing `link` would have to guess which to destroy |
...
| `list` | `[--namespace] [--kind] [--status] [--tag] [--needs-embedding] [--unreferenced] [--direct] [--abstract]` | array of compact summaries |
| `check` | `--text [--namespace] [--tags] [--touches] [--semantic] [--vector --model] [--limit]`, or `--diff` / `--staged` / `--diff-rev <range>` | ranked array of compact candidates — id, namespace, kind, status, short excerpt, `match_kind` (`lexical`/`semantic`/`both`), `rank` (lower is more relevant; scale unspecified and comparable only within one result set), plus `verdict` (`duplicate`/`related`/`weak`) with the evidence behind it: `similarity` where a vector was compared, `shared_facets`, `stale` and `challenged`. Statements and rejections included, distinctly tagged. `--touches <identifier>` retrieves by identifier. `--namespace` narrows to one namespace and its children; omitted, `check` searches the whole corpus, because a decision filed in an area the caller would not have thought to name is exactly the one worth surfacing. Defaults to 10 results; `--limit 0` is unlimited. Full bodies are a deliberate second `get` call. With `--diff` it answers a different question — which recorded decisions bear on a patch — and takes no draft text. `dropped` reports decisions whose last code label the patch removes: the post-image cannot answer what a change took away, so the one patch that makes a decision invisible was the one reporting nothing at all. A label that moved between files is not reported, since the tree is rescanned after the change |
| `embed` | `<id> --vector --model [--force]` | stored vector's id, model, dims, timestamp. Manual escape hatch; `reindex --embed` is the normal path. |
| `audit` | `[--namespace] [--neighbors] [--limit] [--min-score]` | ranked array of candidate conflicting/duplicate pairs, excerpt-only, excluding pairs already adjudicated by a relationship or a dismissal. Pairs sharing an identifier or a source file rank first, with `shared_facets` naming why; incompatible modality is flagged but only breaks ties. The outstanding pair count goes to stderr |
...
| `reindex` | `[--embed]` | counts: added/updated/removed/unchanged, each with the ids behind it — a bare "removed: 1" cannot be acted on, since only requiem knows which file held which record. Also rescans the source tree for labels, unlike the lazy reindex the read path performs. With `--embed`, also fills missing/stale vectors; partial failure persists progress and exits nonzero. The index records a `derivation_version`, and moving it reparses the corpus once, since derived data added — or extracted differently — is otherwise invisible on an existing index forever. |
...
Added after the first trial on a corpus reconstructed from another project: rejections as first-class records (own file, embedded, checked pointers), calibrated verdicts on every `check` result, an identifier facet index with `check --touches`, `brief`, `batch`, a duplicate check inside `add`, dismissals in a verdict store instead of the graph, audit pairing on identifiers with modality demoted to a tiebreak, and `check --diff` with an opt-in per-project gate.
...
- `--abstract` declares that no code can implement a statement — a principle, a process decision, a rule about the corpus itself — excluding it from `--unreferenced`, which would otherwise list it forever. It is an assertion by the author, since requiem cannot tell "nothing implements this" from "nothing can", and a wrong one hides a real gap permanently. So the one case that admits evidence is checked: `audit` reports an abstract statement that turns out to have code referencing it. That catches only the declaration falsified by evidence, never the likelier error — a statement marked abstract to quiet `--unreferenced` when it should have been implemented — so `list --abstract` returns the declarations themselves, making the suppressions reviewable the way a linter's ignore pragmas are. A filter rather than a field on the summary: a bool false for nine rows in ten costs context in every result, to answer a question asked during review and almost never otherwise.
...
One addition tests that line and is worth naming: `check --diff` reports — and an `error` gate fails on — a decision whose last code label the patch *removes*. That is not enforcing coverage. Enforcement would mean judging that a change ought to have carried a label, a judgment about intent; this judges nothing and only reports a fact the patch contains, that a claim someone wrote down was deleted here. A project that never labelled has nothing to drop and never sees it, so the promise below still holds exactly.
