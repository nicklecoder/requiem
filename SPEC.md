...
Where `check` compares one draft against prior decisions, `audit` compares the corpus against itself, surfacing pairs worth a human's attention. Pairs already adjudicated are excluded — by a recorded relationship in either direction, or by a dismissal in the verdict store — so a judged pair stops resurfacing. The count of pairs still outstanding is reported alongside: the backlog refilling as it is worked is by design, but without a total it reads as a queue that grows while you empty it, and one real corpus went from 415 to 425 outstanding pairs after 97 verdicts.
