...
relationships:
    ...
    - to: model/validate-write-tolerate-read
      type: refines
      note: 'see_instead follows exactly that asymmetry: validated when written, never failing the file on read, and reported by audit when it resolves to nothing.'
    - to: model/one-file-per-record
      type: depends_on
      note: 'one-file-per-record names this as one of the failures a shared per-namespace file caused: mv never rewrote a rejection''s see_instead because the rejection was not individually addressable. Checking and rewriting that pointer needs the record to have its own file.'
---
